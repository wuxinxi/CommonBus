...
public class LoopScanTask2 extends Service {
    @Nullable
    @Override
    ...
    public void onCreate() {
        ...
        ThreadScheduledExecutorUtil.getInstance().getService().scheduleAtFixedRate(new Runnable() {
            ...
            public void run() {
                ...
                Log.d("LoopScanTask",
                    "run(LoopScanTask.java:69)"+result);
    ...
    private void verifyCode(String codeResult) {
        ...
        String mac_root_key=wxSdk.get_mac_root_id();
        Log.d("LoopScanTask",
            "verifyCode(LoopScanTask.java:101)mac_root_key="+mac_root_key);
        Log.d("LoopScanTask",
            ...
            if (filterBlackName(open_id)) {
                ...
                if (init == 0 && key_id > 0) {
                    if (key_id == 1) {
                        ...
                        verify = wxSdk.verify(open_id, TestConstant.key_1, TestConstant.tickPrice, TestConstant.scane, TestConstant.scaneType, pos_id, Utils.Random(10),mac_root_key);
                    } else {
                        verify = wxSdk.verify(open_id, TestConstant.key_2, TestConstant.tickPrice, TestConstant.scane, TestConstant.scaneType, TestConstant.pos_id, Utils.Random(10),mac_root_key);
                    }
                    if (verify == 0) {
                        //验证成功
